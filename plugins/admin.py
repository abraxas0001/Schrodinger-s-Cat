...
# Helper: Safe send/edit to handle FloodWait
async def safe_edit(message, text, reply_markup=None):
    try:
        return await message.edit(text, reply_markup=reply_markup)
...
async def add_admins(client: Client, message: Message):
    ...
    if check == len(valid_ids):
        ...
        await safe_edit(pro, f"<b>✅ Admin(s) added successfully:</b>\n\n{admin_list}", reply_markup=reply_markup)
    else:
        await safe_edit(
            pro,
...
async def delete_admins(client: Client, message: Message):
    ...
    if len(admins) == 1 and admins[0].lower() == "all":
        if admin_ids:
            ...
            return await safe_edit(pro, f"<b>⛔️ All admin IDs have been removed:</b>\n{ids}", reply_markup=reply_markup)
        else:
            return await safe_edit(pro, "<b><blockquote>No admin IDs to remove.</blockquote></b>", reply_markup=reply_markup)
    ...
    if admin_ids:
        ...
        await safe_edit(pro, f"<b>⛔️ Admin removal result:</b>\n\n{passed}", reply_markup=reply_markup)
    else:
        await safe_edit(pro, "<b><blockquote>No admin IDs available to delete.</blockquote></b>", reply_markup=reply_markup)
...
async def get_admins(client: Client, message: Message):
    ...
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("ᴄʟᴏsᴇ", callback_data="close")]])
    await safe_edit(pro, f"<b>⚡ Current Admin List:</b>\n\n{admin_list}", reply_markup=reply_markup)


@Bot.on_message(filters.command('backup') & filters.private & admin)
async def manual_backup(client: Bot, message: Message):
    pro = await message.reply("<b><i>ᴘʀᴇᴘᴀʀɪɴɢ ʙᴀᴄᴋᴜᴘ..</i></b>", quote=True)
    try:
        mode = 'json'
        if len(message.command) > 1:
            candidate = message.command[1].lower()
            if candidate in ('json', 'bson'):
                mode = candidate
        await client.daily_backup(chat_id=message.from_user.id, mode=mode)
        await safe_edit(pro, f"<b>✅ Backup ({mode.upper()}) sent!</b>")
    except Exception as e:
        await safe_edit(pro, f"<b>❌ Backup failed: {e}</b>")
